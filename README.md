...
- stable OpenCode Planner / Builder / Reviewer runtime roles generated by the orchestrator;
...
long-horizon model, and Reviewer uses a different model family to reduce correlated self-review
errors. Deterministic tests and policy remain authoritative regardless of model choice. See
[MODEL_ROUTING.md](docs/MODEL_ROUTING.md) for alternatives and local-only routing.
...
cannot be relaxed from project YAML: Planner/Reviewer remain read-only; Builder cannot use `git push`,
`gh`, destructive reset/clean, external directories or nested task delegation.
