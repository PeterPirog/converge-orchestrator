...
- optional shared PostgreSQL registry and LangGraph PostgreSQL checkpoints for production state;
...
```bash
converge run --config /path/to/converge.yaml
```

`converge run` uses the same durable `ScheduledRunController` and control registry as `converge-api`.
If the CLI process is restarted, it reconnects to the single unfinished run for that project instead
of creating duplicate work. Machine-managed CI waits and recoverable checkpoints are reconciled
without becoming HITL interrupts. More than one unfinished run is treated as ambiguous and fails
closed rather than guessing.

`--thread-id` is optional and must be unique when supplied for a new run. Use `--project-id` only when
the CLI must share an explicit control-plane project ID that differs from `project.name`. CLI and API
both honor `CONVERGE_CONTROL_DB` for a non-default shared SQLite registry; PostgreSQL deployments use
