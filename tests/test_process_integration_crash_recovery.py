...
def _config(tmp_path: Path, repo: Path) -> tuple[Path, Path, Path]:
    ...
    requirements.write_text("Integration must recover after process death.\n", encoding="utf-8")
...
def test_service_process_recovers_commit_and_push_without_duplicate_side_effects(
    tmp_path: Path,
) -> None:
    ...
    worker = Path(__file__).parent / "fixtures" / "process_integration_crash_worker.py"
    ...
    assert crashed.returncode == 92, crashed.stdout

    crashed_record = ControlRegistry(registry_path).runs_for_project("chaos-integration")[0]
    assert crashed_record["status"] == "running"
    assert crashed_record["finished_at"] is None
    assert crashed_record["lease_owner"]

    branch = "converge/chaos-integrate-001"
    remote_head_before = _git(repo, "ls-remote", "origin", f"refs/heads/{branch}")
    assert remote_head_before
    pushed_sha_before = remote_head_before.split()[0]
    ...
    record = ControlRegistry(registry_path).runs_for_project("chaos-integration")[0]
    assert record["status"] == "completed"
    assert record["finished_at"] is not None
    assert record["lease_owner"] is None
    assert (state_dir / "integration-chaos-attempts.txt").read_text(encoding="utf-8") == "2"

    candidate = worktree_dir / "chaos-integrate-001"
    assert (candidate / "candidate.txt").read_text(encoding="utf-8") == (
        "candidate survives integration crash\n"
    )
    assert _git(candidate, "status", "--porcelain") == ""
    assert _git(candidate, "rev-parse", "HEAD") == pushed_sha_before

    remote_head_after = _git(repo, "ls-remote", "origin", f"refs/heads/{branch}")
    assert remote_head_after.split()[0] == pushed_sha_before

    origin_log = _git(origin, "log", "--format=%H", branch)
    assert origin_log.splitlines()[0] == pushed_sha_before
    assert len(origin_log.splitlines()) == 2
    ...
    branches = _git(repo, "branch", "--list", branch).splitlines()
